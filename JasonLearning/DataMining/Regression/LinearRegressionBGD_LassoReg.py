...
def computeCost_Reg(X, y, w,lambda_):
    """
    计算线性回归模型的代价函数。损失函数带正则化项.    
    参数：
    ...
    return np.sum(inner) / 2  +  (lambda_ * np.sum(w ** 2))  # 对平方和进行求和并除以样本数量的两倍，得到代价函数的值
...
def batch_gradientDescent_Reg(X, y, w, alpha, count,lambda_):
    ...
    for i in range(count):
        ...
        # 计算当前代价函数值并添加到列表中
        cost = computeCost_Reg(X, y, w,lambda_)
        costs.append(cost)
...
# 计算初始代价函数 j0
j0 = computeCost_Reg(X, y, w,lambda_)
# print(j0)

# 设置学习速率和迭代次数
alpha = 0.01
iters = 2000
w, cost = batch_gradientDescent_Reg(X, y, w, alpha, iters,lambda_)#返回更新后的参数向量g和损失值数组cost。

# 计算拟合的训练模型的代价函数
j1 = computeCost_Reg(X, y, w,lambda_) 
print(j1) 
